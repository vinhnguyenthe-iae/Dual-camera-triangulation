...
from src.detection.mediapipe_adapter import MediaPipeAdapter
from src.triangulation.triangulate import triangulate_points
from src.visualization.pg_viewer import PGHandViewer
...
def get_first_hand_keypoints(adapter, frame):
    pts = adapter.get_first_hand_keypoints_px(frame)
    if pts is None:
        return None
...
def main():
    ...
    if len(sys.argv) >= 3:
        try:
            ...
            print("Usage: python src/triangulation/dual_live_triangulate.py <index1> <index2>")
            sys.exit(1)
    ...
    try:
        while True:
            ...
            pts1 = get_first_hand_keypoints(detector, f1)
            pts2 = get_first_hand_keypoints(detector, f2)
