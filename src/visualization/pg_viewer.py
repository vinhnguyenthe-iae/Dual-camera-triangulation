...
from .hand_skeleton import HAND_CONNECTIONS
...
class PGHandViewer:
    def __init__(self):
        ...
        self.view.setWindowTitle('3D Hand Triangulation')
        ...
        # Points
        self.scatter = gl.GLScatterPlotItem(pos=np.zeros((0, 3)), size=5, color=(1.0, 0.3, 0.3, 1.0))
        self.view.addItem(self.scatter)

        # Lines per connection
        self.line_items = []
        for _ in HAND_CONNECTIONS:
            line = gl.GLLinePlotItem(pos=np.zeros((2, 3)), color=(0.2, 1.0, 0.2, 1.0), width=2, antialias=True, mode='line_strip')
    ...
    def update(self, points_3d: np.ndarray):
        ...
        for (edge_idx, (i, j)) in enumerate(HAND_CONNECTIONS):
